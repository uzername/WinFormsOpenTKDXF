...
namespace WinFormsOpenTKDXF
{
    public partial class UserControlWinFormsDXF : UserControl
    {
        ...
        private void generateVerticesGrid(int szX, int szY, double dimGrid)
        {
            ...
            _verticesGrid[1] = (float)((dimGrid * szY) / 2.0);
            _verticesGrid[2] = 0.0f;

            _verticesGrid[3] = (float)((dimGrid * szX) / 2.0);
            _verticesGrid[4] = -(float)((dimGrid * szY) / 2.0);
            _verticesGrid[5] = 0.0f;

            _verticesGrid[6] = -(float)((dimGrid * szX) / 2.0);
            _verticesGrid[7] = -(float)((dimGrid * szY) / 2.0);
            _verticesGrid[8] = 0.0f;

            _verticesGrid[9] = -(float)((dimGrid * szX) / 2.0);
            _verticesGrid[10] = (float)((dimGrid * szY) / 2.0);
            _verticesGrid[11] = 0.0f;

            // Lines: 1-2, 2-3, 3-4, 4-1
            _indicesGrid = new uint[] {
            0, 1, 3,
            1, 2, 3};
        }
        ...
        private void scenePreInit()
        {
            ...
            GL.BindVertexArray(_vertexArrayObject);
                ...
                // Stride and Offset are just sort of glossed over for now
            ...
            _shader = new Shader("Graphics3D/Shaders/shaderGrid.vert", "Graphics3D/Shaders/shaderGrid.frag");
        ...
        private void Render()
        {
            ...
            var model = Matrix4.Identity ; //identifies transformations of geometry applied before rendering
            _shader.SetMatrix4("model", model);
            _shader.SetMatrix4("view", _camera.GetViewMatrix());
            _shader.SetMatrix4("projection", _camera.GetProjectionMatrix());

            GL.DrawElements(PrimitiveType.Triangles, _indicesGrid.Length, DrawElementsType.UnsignedInt, 0);
        ...
        private void glControl1_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if ((_firstMove)&&(e.Button == MouseButtons.Left)) // This bool variable is initially set to true.
            {
